...
class MainActivity : ComponentActivity() {
    ...
    private val textsToEmbed =
        mapOf(
            ...
            "Doc 1" to
                "파이썬에서 리스트(List)와 튜플(Tuple)의 가장 결정적인 차이점은 '변경 가능성'(Mutability)입니다. 리스트는 생성된 후에 원소를 추가, 삭제, 수정하는 것이 자유로운 '변경 가능한(mutable)' 객체입니다. 반면, 튜플은 한 번 생성되면 그 내용을 바꿀 수 없는 '변경 불가능한(immutable)' 객체입니다.",
            "Doc 2" to
                "리스트(List)는 파이썬의 대표적인 순서형 자료구조로, 대괄호 `[]`를 사용하여 생성합니다. `append()` 메소드로 새로운 원소를 추가하거나, `del` 키워드로 특정 인덱스의 원소를 삭제하는 등 다양한 연산을 통해 내용을 자유롭게 수정할 수 있습니다.",
            "Doc 3" to
                "튜플(Tuple)은 소괄호 `()`를 사용하여 정의하며, 한 번 생성되면 내부 원소를 변경할 수 없습니다. 이처럼 데이터의 불변성이 보장되어야 할 때, 예를 들어 함수의 반환 값으로 여러 값을 안전하게 전달하고 싶을 때 유용하게 사용됩니다.",
            "Doc 4" to
                "딕셔너리(Dictionary)는 'Key'와 'Value'를 한 쌍으로 묶어 관리하는 자료구조입니다. `{}`를 사용하며, 순서가 없는 것이 특징입니다. 각 값에 고유한 키를 부여하여 데이터를 효율적으로 탐색하고 관리할 수 있습니다.",
            "Doc 5" to
                "`for` 반복문은 리스트, 튜플, 문자열 등 순회 가능한(iterable) 객체의 원소를 하나씩 차례대로 방문하여 코드 블록을 실행하는 데 사용됩니다. 데이터를 순차적으로 처리해야 할 때 필수적인 구문입니다.",
